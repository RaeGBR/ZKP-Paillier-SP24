...
describe("Crypto Library - Utils", () => {
  ...
  describe("Random Utils", () => {
    ...
    it("Integer", async () => {
      const int1 = await wasm.Utils.randomInt(length);
      const int2 = await wasm.Utils.randomInt(length);
    ...
    it("Integer with Seed", async () => {
      ...
      const int1 = await wasm.Utils.randomIntWithSeed(length, seed);
      assert.equal(int1.toHex(), answer);
      assert.isAtMost(int1.toHex().length / 2, length);
    });

    it("Hex", async () => {
      const h1 = await wasm.Utils.randomHex(length);
      const h2 = await wasm.Utils.randomHex(length);
    ...
    it("Hex with Seed", async () => {
      ...
      const h1 = await wasm.Utils.randomHexWithSeed(length, seed);
  ...
  describe("Hash Utils", () => {
    it("SHA256", () => {
      ...
      const result = wasm.Utils.calcHash(message);
      assert.equal(
      ...
      const result2 = wasm.Utils.calcHash(message2);
      assert.equal(
  ...
  describe("EC Utils", () => {
    it("ECDSA", () => {
      ...
      const s1 = wasm.Utils.calcSignature(curve, p1.getPrivateKey(), message);
      const s2 = wasm.Utils.calcSignature(curve, p2.getPrivateKey(), message);
      const s3 = wasm.Utils.calcSignature(curve, p1.getPrivateKey(), message);
      const s4 = wasm.Utils.calcSignature(curve, p2.getPrivateKey(), message);
      ...
      assert.equal(
        wasm.Utils.verifySignature(curve, p1.getPublicKey(), message, s1),
        true
      );
      assert.equal(
        wasm.Utils.verifySignature(curve, p2.getPublicKey(), message, s2),
        true
      );
      assert.equal(
        wasm.Utils.verifySignature(curve, p1.getPublicKey(), message, s3),
        true
      );
      assert.equal(
        wasm.Utils.verifySignature(curve, p2.getPublicKey(), message, s4),
        true
      );

      assert.equal(
        wasm.Utils.verifySignature(curve, p1.getPublicKey(), message, s2),
        false
      );
      assert.equal(
        wasm.Utils.verifySignature(curve, p2.getPublicKey(), message, s1),
        false
      );
      assert.equal(
        wasm.Utils.verifySignature(curve, p1.getPublicKey(), fakeMessage, s1),
        false
      );
      assert.equal(
        wasm.Utils.verifySignature(curve, p2.getPublicKey(), fakeMessage, s2),
        false
      );
    });
  });
